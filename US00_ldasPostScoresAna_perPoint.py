...
#files = sorted(glob.glob('C:/Users/antho/Documents/SXVGO1/CustomPostProcessing/Data/US00/ldasPostScores/*scores_for_all_points*'),key=numericalSort)
files = sorted(glob.glob('C:/Users/antho/Documents/SXVGO1/CustomPostProcessing/Data/US00/ldasPostScores/V4/NEast*per_point*'),key=numericalSort)
for i in range(len(files)):
    ...
    if 'FLCGPP' in exp:
        df = xr.open_dataarray(files[i])
        if '_LAI_' in exp:
            gpp_lai_R = np.nanmean(df[1][0].values,axis=1)
        if '_LAISSM_' in exp:
            gpp_laissm_R = np.nanmean(df[1][0].values,axis=1)
        if '_VOD10_' in exp:
            gpp_vod10_R = np.nanmean(df[1][0].values,axis=1)
        if '_OL_' in exp:
            gpp_ol_R = np.nanmean(df[1][0].values,axis=1)
        if '_VOD_' in exp:
            gpp_vod_R = np.nanmean(df[1][1].values,axis=1)
        if '_VODSSM_' in exp:
            gpp_vodssm_R = np.nanmean(df[1][1].values,axis=1)
        if '_SSM_' in exp:
            gpp_ssm_R = np.nanmean(df[1][1].values,axis=1)

    if 'ET_04' in exp:
        df = xr.open_dataarray(files[i])
        if '_LAI_' in exp:
            et_lai_R = np.nanmean(df[1][0].values,axis=1)
        if '_LAISSM_' in exp:
            et_laissm_R = np.nanmean(df[1][0].values,axis=1)
        if '_VOD10_' in exp:
            et_vod10_R = np.nanmean(df[1][0].values,axis=1)
        if '_OL_' in exp:
            et_ol_R = np.nanmean(df[1][0].values,axis=1)
        if '_VOD_' in exp:
            et_vod_R = np.nanmean(df[1][1].values,axis=1)
        if '_VODSSM_' in exp:
            et_vodssm_R = np.nanmean(df[1][1].values,axis=1)
        if '_SSM_' in exp:
            et_ssm_R = np.nanmean(df[1][1].values,axis=1)

    if 'SSM_COMBINED' in exp:
        df = xr.open_dataarray(files[i])
        if '_LAI_' in exp:
            ssm_lai_R = np.nanmean(df[1][0].values,axis=1)
        if '_LAISSM_' in exp:
            ssm_laissm_R = np.nanmean(df[1][0].values,axis=1)
        if '_VOD10_' in exp:
            ssm_vod10_R = np.nanmean(df[1][0].values,axis=1)
        if '_OL_' in exp:
            ssm_ol_R = np.nanmean(df[1][0].values,axis=1)
        if '_VOD_' in exp:
            ssm_vod_R = np.nanmean(df[1][1].values,axis=1)
        if '_VODSSM_' in exp:
            ssm_vodssm_R = np.nanmean(df[1][1].values,axis=1)
        if '_SSM_' in exp:
            ssm_ssm_R = np.nanmean(df[1][1].values,axis=1)
