...
imgdir = '/cnrm/vegeo/muciaa/Images/US00/VOD_Comparisons/Patch/V2/'
perc = 0.5
eco = "ECOII"
ecol = "ECOCLIMAP II"
match = "matched"
...
axs[1,1].plot(ext9,line9,color='purple',linestyle='--')
...
axs[1,0].set_xlabel('LAI [m^2/m^-2]',color='green')
axs[1,0].set_ylabel('Matched VODC [m^2/m^-2]',color='red')
...
axs.set_title('LAI vs Matched VODX\nVegetation > {0}%\n{1}'.format(round(perc*100),ecol),fontsize=16)
axs.set_xlim(0,6.5)
axs.set_ylim(0,6.5)
xlims=(0,6.5)
...
axs.set_ylabel('Matched VODX [m^2/m^-2]',color='red')
#plt.show()
...
axs.set_title('LAI vs Matched VODC\nVegetation > {0}%\n{1}'.format(round(perc*100),ecol),fontsize=16)
...
axs.set_xlabel('LAI [m^2/m^-2]',color='green')
axs.set_ylabel('Matched VODC [m^2/m^-2]',color='red')
...
grass_x = ['Grasslands',r_grass_x[0],r_grass_win_x[0],r_grass_spr_x[0],r_grass_sum_x[0],r_grass_aut_x[0]]
datax = [veg_x,decid_x,conif_x,c3_x,grass_x]
...
datac = [veg_c,decid_c,conif_c,c3_c,grass_c]
