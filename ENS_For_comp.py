...
while True:
    response = raw_input('Would you like Rain/SWI/LAI Time Series? (Y)/(N):  ')
    if response != "Y" and response != "N":
        print 'Error: Input Not Expected. Please enter (Y) or (N):  '
    elif response == 'Y':
        plt.title('ENS Rainfall',fontsize=16)
        for i in xrange(len(rain)):
            plt.plot(rain.iloc[i].mean(axis=1),label='ENS'+str(i+1))
        plt.plot(rain0.mean(axis=1),label='ENS CTRL',color='black',linewidth=2)
        plt.legend(loc='upper right',fontsize=8)
        plt.show()

        plt.title('ENS SWI OL',fontsize=16)
        for i in xrange(len(swi)):
            plt.plot(swi.iloc[i].mean(axis=1),label='ENS'+str(i+1))
        ...
        plt.title('ENS GPP OL',fontsize=16)
        for i in xrange(len(swi)):
            plt.plot(gpp.iloc[i].mean(axis=1),label='ENS'+str(i+1))
        plt.legend(loc='upper right',fontsize=8)
        plt.show()
        
        plt.title('ENS EVAP OL',fontsize=16)
        for i in xrange(len(swi)):
...
#plt.title('ENS Rainfall Variance',fontsize=16)
...
#rstd = rainp.std(axis=0).mean(axis=0)
#V1_R = rstd.reshape(52,80)

#var_m = var.mean(axis=0)
#V1 = var_m.reshape(52,80)

#D = V1_R - V1
#plt.title("Annual Rainfall (Standard Deviation / Mean) Diff - 25km")
#map = Basemap(projection='cyl', llcrnrlon=-105, llcrnrlat=37, urcrnrlon=-85, urcrnrlat=50,resolution='i')
#map.drawstates(linewidth=1.8)
#map.drawcoastlines(1.8)
#map.drawmapboundary()
#map.drawcountries(1.8)
#map.drawrivers()
#im = map.imshow(D,interpolation='none',cmap='YlGn')
#plt.colorbar()
...
#plt.title("Annual Rainfall (Standard Deviation / Mean) OLD - 25km")
#map = Basemap(projection='cyl', llcrnrlon=-105, llcrnrlat=37, urcrnrlon=-85, urcrnrlat=50,resolution='i')
#map.drawstates(linewidth=1.8)
#map.drawcoastlines(1.8)
#map.drawmapboundary()
#map.drawcountries(1.8)
#map.drawrivers()
#im = map.imshow(V1,interpolation='none',cmap='YlGn')
#plt.colorbar()

#plt.show()

#plt.title("Annual Rainfall (Standard Deviation / Mean) NEW - 25km")
#map = Basemap(projection='cyl', llcrnrlon=-105, llcrnrlat=37, urcrnrlon=-85, urcrnrlat=50,resolution='i')
#map.drawstates(linewidth=1.8)
#map.drawcoastlines(1.8)
#map.drawmapboundary()
#map.drawcountries(1.8)
#map.drawrivers()
#im = map.imshow(V1_R,interpolation='none',cmap='YlGn')
#plt.colorbar()
#plt.show()
