...
corfiles = sorted(glob.glob('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/*R.PData'),key=numericalSort)
pfiles = sorted(glob.glob('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/*P.PData'),key=numericalSort)
...
wg3 = pd.DataFrame(l3,columns=['Exp','Correlation','{0} CI +/-'.format(1-alpha),'# of Stations'])

l20 = list(zip(WG_20,cWG_20,mWG_20,sWG_20))
wg20 = pd.DataFrame(l20,columns=['Exp','Correlation','{0} CI +/-'.format(1-alpha),'# of Stations'])

l6 = list(zip(WG6,cWG6,mWG6,sWG6))
wg6 = pd.DataFrame(l6,columns=['Exp','Correlation','{0} CI +/-'.format(1-alpha),'# of Stations'])

l8 = list(zip(WG8,cWG8,mWG8,sWG8))
wg8 = pd.DataFrame(l8,columns=['Exp','Correlation','{0} CI +/-'.format(1-alpha),'# of Stations'])
...
x = 0,1,2,3,4,5
ticks = ['EKF LAI','EKF LAI SSM','EKF SSM','EKF VOD','EKF VOD SSM','OL']
