...
exp1_list = ('ol','ekf_ssm','ekf_lai_ssm','ekf_vod10')
exp2_list = ('ekf_lai','ekf_vod','ekf_vod_ssm','ekf_vod10')
...
for m in range(len(exp1_list)):
    ...
    for i in range(len(depth)):
        ...
        P_win_ol = list() ; P_spr_ol = list() ; P_sum_ol = list() ; P_aut_ol = list()
        P_win_ekf = list() ; P_spr_ekf = list() ; P_sum_ekf = list() ; P_aut_ekf = list()
        
        R_grow_ol = list(); R_grow_ekf = list()
        P_grow_ol = list(); P_grow_ekf = list()
            ...
            if (lat_1D[-1,0] >= lat[coord] >= lat_1D[0,0]) and (lon_1D[0,-1] >= lon[coord] >= lon_1D[0,0]):
                ...
                if (len(obs_05bis.resample('D').mean()[~np.isnan(obs_05bis.resample('D').mean())]) > 100):
                    ...
                    if len(wg3_ekf_bis[~np.isnan(wg3_ekf_bis)])>100.:
                        ...
                        grow_obs = obs_05bis_DD.loc[(obs_05bis_DD.index.month==4)+(obs_05bis_DD.index.month==5)+(obs_05bis_DD.index.month==6)+(obs_05bis_DD.index.month==7)+(obs_05bis_DD.index.month==8)+(obs_05bis_DD.index.month==9)]
                        win_obs = obs_05bis_DD.loc[(obs_05bis_DD.index.month==12)+(obs_05bis_DD.index.month==1)+(obs_05bis_DD.index.month==2)]
                        spr_obs = obs_05bis_DD.loc[(obs_05bis_DD.index.month==3)+(obs_05bis_DD.index.month==4)+(obs_05bis_DD.index.month==5)]
                        sum_obs = obs_05bis_DD.loc[(obs_05bis_DD.index.month==6)+(obs_05bis_DD.index.month==7)+(obs_05bis_DD.index.month==8)]
                        aut_obs = obs_05bis_DD.loc[(obs_05bis_DD.index.month==9)+(obs_05bis_DD.index.month==10)+(obs_05bis_DD.index.month==11)]
                        ...
                        if len(win_obs[~np.isnan(win_obs)])>50:
                            R_win_ol.append(correlation((win_ol).values,win_obs.values)[0])
                            R_win_ekf.append(correlation((win_ekf).values,win_obs.values)[0])
                            P_win_ol.append(correlation((win_ol).values,win_obs.values)[1])
                            P_win_ekf.append(correlation((win_ekf).values,win_obs.values)[1])
                            
                        if len(spr_obs[~np.isnan(spr_obs)])>50:
                            R_spr_ol.append(correlation((spr_ol).values,spr_obs.values)[0])
                            R_spr_ekf.append(correlation((spr_ekf).values,spr_obs.values)[0])
                            P_spr_ol.append(correlation((spr_ol).values,spr_obs.values)[1])
                            P_spr_ekf.append(correlation((spr_ekf).values,spr_obs.values)[1])
                          
                        if len(sum_obs[~np.isnan(sum_obs)])>50:
                            R_sum_ol.append(correlation((sum_ol).values,sum_obs.values)[0])
                            R_sum_ekf.append(correlation((sum_ekf).values,sum_obs.values)[0])
                            P_sum_ol.append(correlation((sum_ol).values,sum_obs.values)[1])
                            P_sum_ekf.append(correlation((sum_ekf).values,sum_obs.values)[1])
                            
                        if len(aut_obs[~np.isnan(aut_obs)])>50:
                            R_aut_ol.append(correlation((aut_ol).values,aut_obs.values)[0])
                            R_aut_ekf.append(correlation((aut_ekf).values,aut_obs.values)[0])  
                            P_aut_ol.append(correlation((aut_ol).values,aut_obs.values)[1])
                            P_aut_ekf.append(correlation((aut_ekf).values,aut_obs.values)[1])
                        
                        if len(grow_obs[~np.isnan(grow_obs)])>50:
                            R_grow_ol.append(correlation((grow_ol).values,grow_obs.values)[0])
                            R_grow_ekf.append(correlation((grow_ekf).values,grow_obs.values)[0])
                            P_grow_ol.append(correlation((grow_ol).values,grow_obs.values)[1])
                            P_grow_ekf.append(correlation((grow_ekf).values,grow_obs.values)[1])      
                        ...
                        R_win_ol.append(np.nan)
                        R_spr_ol.append(np.nan)
                        R_sum_ol.append(np.nan)
                        R_aut_ol.append(np.nan)
                        
                        R_win_ekf.append(np.nan)
                        R_spr_ekf.append(np.nan)
                        R_sum_ekf.append(np.nan)
                        R_aut_ekf.append(np.nan)
                        
                        P_win_ol.append(np.nan)
                        P_spr_ol.append(np.nan)
                        P_sum_ol.append(np.nan)
                        P_aut_ol.append(np.nan)
                        
                        P_win_ekf.append(np.nan)
                        P_spr_ekf.append(np.nan)
                        P_sum_ekf.append(np.nan)
                        P_aut_ekf.append(np.nan)
                        
                        R_grow_ol.append(np.nan)
                        R_grow_ekf.append(np.nan)
                        
                        P_grow_ol.append(np.nan)
                        P_grow_ekf.append(np.nan)
        ...
        R_win_ol = pd.DataFrame(R_win_ol)
        R_spr_ol = pd.DataFrame(R_spr_ol)
        R_sum_ol = pd.DataFrame(R_sum_ol)
        R_aut_ol = pd.DataFrame(R_aut_ol)
        
        R_win_ekf = pd.DataFrame(R_win_ekf)
        R_spr_ekf = pd.DataFrame(R_spr_ekf)
        R_sum_ekf = pd.DataFrame(R_sum_ekf)
        R_aut_ekf = pd.DataFrame(R_aut_ekf)
        
        P_win_ol = pd.DataFrame(P_win_ol)
        P_spr_ol = pd.DataFrame(P_spr_ol)
        P_sum_ol = pd.DataFrame(P_sum_ol)
        P_aut_ol = pd.DataFrame(P_aut_ol)
        
        P_win_ekf = pd.DataFrame(P_win_ekf)
        P_spr_ekf = pd.DataFrame(P_spr_ekf)
        P_sum_ekf = pd.DataFrame(P_sum_ekf)
        P_aut_ekf = pd.DataFrame(P_aut_ekf)
        
        R_grow_ol = pd.DataFrame(R_grow_ol)
        R_grow_ekf = pd.DataFrame(R_grow_ekf)
        
        P_grow_ol = pd.DataFrame(P_grow_ol)
        P_grow_ekf = pd.DataFrame(P_grow_ekf)
        ...
        R_win_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_win_R.PData'.format(exp1,wg,dep))
        R_win_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_win_R.PData'.format(exp2,wg,dep))
        R_spr_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_spr_R.PData'.format(exp1,wg,dep))
        R_spr_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_spr_R.PData'.format(exp2,wg,dep))
        R_sum_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_sum_R.PData'.format(exp1,wg,dep))
        R_sum_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_sum_R.PData'.format(exp2,wg,dep))
        R_aut_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_aut_R.PData'.format(exp1,wg,dep))
        R_aut_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_aut_R.PData'.format(exp2,wg,dep))

        P_win_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_win_P.PData'.format(exp1,wg,dep))
        P_win_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_win_P.PData'.format(exp2,wg,dep))
        P_spr_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_spr_P.PData'.format(exp1,wg,dep))
        P_spr_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_spr_P.PData'.format(exp2,wg,dep))
        P_sum_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_sum_P.PData'.format(exp1,wg,dep))
        P_sum_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_sum_P.PData'.format(exp2,wg,dep))
        P_aut_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_aut_P.PData'.format(exp1,wg,dep))
        P_aut_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_aut_P.PData'.format(exp2,wg,dep))
        
        R_grow_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_grow_R.PData'.format(exp1,wg,dep))
        R_grow_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_grow_R.PData'.format(exp2,wg,dep))
        
        P_grow_ol.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_grow_P.PData'.format(exp1,wg,dep))
        P_grow_ekf.to_pickle('/cnrm/vegeo/muciaa/ldas_chain_python/ldas_curr/CustomPostProcessing/Data/US00/USCRN/Seasonal/{0}_{1}_{2}_grow_P.PData'.format(exp2,wg,dep))
